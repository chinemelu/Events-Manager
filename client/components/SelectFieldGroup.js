...
class SelectFieldGroup extends React.Component {
  constructor(props){
    super(props)
    this.state = {
      centerId:  this.props.selectBoxOption.filter(center => center.id === this.props.centerId).name
    }
  }

  render() {  
    const centers = this.props.selectBoxOption.filter(center => center.id === this.props.centerId)
    ...
    return (
      ...
      <select  value={this.props.selected} className="form-control" id="exampleSelect1" name={this.props.field} onChange={this.props.onChange}>
